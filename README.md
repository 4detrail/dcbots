...
### 4. Google Gemini API Key al (ücretsiz)
https://aistudio.google.com/apikey → Google hesabınla giriş yap → **Create API Key** → `.env` içine `GEMINI_API_KEY`

Ücretsiz katmanda dakikada belirli sayıda istek sınırı var (Gemini 1.5 Flash modeli kullanılıyor, en hızlı ve en ucuz/ücretsiz seçenek). Sunucun çok yoğunsa ileride ücretli katmana geçmen gerekebilir, ama başlangıç için tamamen yeterli.
...
3. Ayarlar:
   ...
   - **Plan:** Free
4. **Environment** sekmesinden `.env` dosyandaki TÜM değişkenleri tek tek ekle (`DISCORD_TOKEN`, `CLIENT_ID`, `GEMINI_API_KEY`, `FIREBASE_SERVICE_ACCOUNT_JSON`, `FIREBASE_PROJECT_ID`)
